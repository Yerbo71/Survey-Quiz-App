import { useState } from "react";
import { resultInitialState } from "../../constants";
import "../../components/Quiz/Quiz.scss";
import AnswerTimer from "../AnswerTimer/AnswerTimer";
import QuizTimer from "../QuizTimer/QuizTimer";
import 'chart.js/auto';
import { Doughnut } from "react-chartjs-2";

const Quiz = ({questions}) => {
    const [currentQuestion, setCurrentQuestion] = useState(0);
    const [answerIdx, setAnswerIdx] = useState(null);
    const [answer, setAnswer] = useState(null);
    const [result, setResult] = useState(resultInitialState);
    const [showResult, setShowResult] = useState(false);
    const [showAnswerTimer, setShowAnswerTimer] = useState(true);
    const [quizTimer, setQuizTimer] = useState(questions.length * 10);

    /* For Doughnut Graphick*/
    const data = {
        labels: ["Correct", "Wrong",],
        datasets: [
          {
            data: [result.correctAnswer,result.wrongAnswer],
            backgroundColor: [
                "rgb(122,231,125)",
                "rgb(236,107,109)",
            ],
            hoverBackgroundColor: ["#047516", "#ab021f",]
          }
        ],
       
        plugins: {
          labels: {
            render: "percentage",
            fontColor: ["green","red"],
            precision: 2
          },
        },
         text: "23%",
      };
      


    const {question, choices, correctAnswer} = questions[currentQuestion];

    const onAnswerClick = (answer, index) =>{
        setAnswerIdx(index);
        if (answer === correctAnswer){
            setAnswer(true);
        }
        else{
            setAnswer(false);
        }

    }

    const onClickNext = (finalAnswer) => {
        setAnswerIdx(null);
        setShowAnswerTimer(false);
        setResult((prev) => 
        finalAnswer
             ? {
                ...prev,
                score: prev.score + 5,
                correctAnswer: prev.correctAnswer + 1
            } : {
                ...prev,
                wrongAnswer: prev.wrongAnswer + 1
            }
        );
        if(currentQuestion !== questions.length - 1){
            setCurrentQuestion((prev) => prev + 1);

        }
        else {
            setCurrentQuestion(0);
            setShowResult(true);
        }

        setTimeout(()=>{
            setShowAnswerTimer(true);
        })
    };

    const onClickPrevious = () => {
        if(currentQuestion > 0){
            setCurrentQuestion((prev) => prev - 1);
        }
    };


    const handleQuestionButtonClick = (questionIndex) => {
        setCurrentQuestion(questionIndex);
        setAnswerIdx(null);
        setAnswer(null);
        setShowAnswerTimer(true);
      };

      
    const onTryAgain = () => {
        setResult(resultInitialState);
        setShowResult(false);
    }
    const handleTimeUp = () => {
        setAnswer(false);
        onClickNext(false);
        setShowResult(false);
    }

    const handleShowFinish = () => {
        setShowResult(true);
      };
    
    return(
        <div className="quiz-container">
            {!showResult ? (
            <>
            {showAnswerTimer && <AnswerTimer duration={10} onTimeUp={handleTimeUp} />}
            <QuizTimer duration={quizTimer} onTimeUp={handleShowFinish}/>
            <span className="active-question-no">{currentQuestion + 1}</span>
            <span className="total-question">/{questions.length}</span>
            <h2>{question}</h2>
            <ul>
                {
                    choices.map((answer, index) => (
                        <li 
                        onClick={() => onAnswerClick(answer, index)}
                        key={answer}
                        className={answerIdx === index ? "selected-answer" : null}
                        >
                            {answer}
                        </li>
                    ))
                }
            </ul>
            <div className="footer">

                <button onClick={() => onClickPrevious()} style={{marginRight:"15px"}} disabled={currentQuestion === 0}>Previous</button>

                <button onClick={() => onClickNext(answer)} disabled={answerIdx === null}>
                    {currentQuestion === questions.length - 1 ? "Finish" : "Next"}
                </button>
            </div>
            <div className="question-buttons">
              {questions.map((_, index) => (
                <button
                  key={index}
                  onClick={() => handleQuestionButtonClick(index)}
                  className={currentQuestion === index ? "active" : ""}
                >
                  {index + 1}
                </button>
              ))}
              </div>
            </>) : <div className="result">
                <h3>Result</h3>
                <p>
                    Total Questions:<span>{questions.length}</span>
                </p>
                <p>
                    Total Score:<span>{result.score}</span>
                </p>
                <p>
                    Correct Answers:<span>{result.correctAnswer}</span>
                </p>
                <p>
                    Wrong Answers:<span>{result.wrongAnswer}</span>
                </p>
                <Doughnut data={data} style={{
                    padding: "20px"
                }}/>
                <button onClick={onTryAgain}>Try again</button>
                </div>}
        </div>
    )

}
export default Quiz;